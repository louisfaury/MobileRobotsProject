...
We implemented an Extended Kalman Filter for localization, based on odometry and landmark-based triangulation. Path-planning is based on a A* algorithm running on a grid decomposed map. We also implemented path abstractions that allow the robot to follow a trajectory while constantly respecting its speed and acceleration constraints. Strategy is based on a finite state machine, helped by a genetically-enhanced neural network for decision tasks. 
